...
void Exportar_Reg_Trabalho(REG_TRABALHO **reg_trabalho, FILE *fp) {
    ...
    if (reg_trabalho_contador >= 1) {
        ...
        if (fp == NULL) {
            ...
            fprintf(fp, "Codigo Funcionario,Contador,");
            ...
            for (i = 0; i < reg_trabalho_contador; i++) {
                ...
                if ((*reg_trabalho)[i].data_trabalho.mes == mes &&
                    ...
                    fprintf(fp, "%i,%i,", (*reg_trabalho)[i].codigo_funcionario,
                            (*reg_trabalho)[i].contador);
...
void Menu_Reg_Trabalho(FUNCIONARIO **funcionarios, REG_TRABALHO **reg_trabalho, FILE *fp) {

    int menu;

    do {
        printf("\nREGISTRO DE TRABALHO CONTADOR%d", (*reg_trabalho)[0].contador);
        printf("\n\nMENU_REGISTRO TRABALHO");
        ...
        switch (menu) {
            ...
            case 7:Funcionario_Excelente(funcionarios, reg_trabalho, fp);
                logMsg("Funcionario do Mes", FILENAME_LOG);
                break;
...
void Carregar_Reg_Trabalho(REG_TRABALHO **reg_trabalho, FILE *fp) {
    int returno = 0;
    int contador = 0;
    ...
    if (fp != NULL) {
        ...
        printf("CONTADOR REG TRABALHO: %i", contador);
